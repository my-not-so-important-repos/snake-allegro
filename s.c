...
char	tabuleiro[33][23], x, y, tamanho_cobra, direcao, vidas, proximox, proximoy, proximo_bitmap;
BITMAP	*chao, *parede, *maca, *cabecacima, *cabecabaixo, *cabecaesquerda, *cabecadireita, *caudacima, *caudabaixo, *
	caudaesquerda, *caudadireita, *vertebrahorizontal, *vertebravertical, *vertebrasuperiordireita, *
		vertebrasuperioresquerda, *vertebrainferiordireita, *vertebrainferioresquerda;
struct vertebra
{
	char	x, y, bitmap;
} cobra[MAX_COBRA];
...
int main()
{
	...
	//Abrir o arquivo que cont‚m as informa‡äes do mapa
	...
	while (vidas)
	{
		...
		switch (tecla)
		{
			...
			case DIREITA:
			case ESQUERDA:
				if ((direcao + tecla) != 152)
				{					//a soma das setas opostas ‚ igual a 152
					direcao = tecla;
				}
				break;
...
void desenhar_cobra(char operacao)
{
	switch (operacao)
	{
		case DESENHAR_PEDACO_COBRA:
			...
			{
				case '':	//24
					blit(caudacima, screen, 0, 0, (cobra[tamanho_cobra - 1].x - 1) * 32, (cobra[tamanho_cobra - 1].y - 1) * 32,
						 parede->w, parede->h);
					break;

				case '':	//25
					blit(caudabaixo, screen, 0, 0, (cobra[tamanho_cobra - 1].x - 1) * 32, (cobra[tamanho_cobra - 1].y - 1) * 32,
						 parede->w, parede->h);
					break;

				case 26:	// eu nao posso colocar o caracter por que ele ‚ o EOF, o compilador vai entender que aqui ‚ o fim do c¢digo fonte
					blit(caudadireita, screen, 0, 0, (cobra[tamanho_cobra - 1].x - 1) * 32, (cobra[tamanho_cobra - 1].y - 1) * 32,
						 parede->w, parede->h);
					break;

				case '':	//27
					blit(caudaesquerda, screen, 0, 0, (cobra[tamanho_cobra - 1].x - 1) * 32, (cobra[tamanho_cobra - 1].y - 1) * 32,
						 parede->w, parede->h);
					break;
			}
			break;
		...
		case APAGAR_PEDACO_COBRA:
			...
			blit(chao, screen, 0, 0, (cobra[tamanho_cobra - 1].x - 1) * 32, (cobra[tamanho_cobra - 1].y - 1) * 32, chao->w,
				 chao->h);			//apagar o rabo
			break;
...
/* */
void cobra_init(char posicaoinicialx, char posicaoinicialy)
{
	/*~~~~~~~~~~~~~~*/
	unsigned short	i;
	/*~~~~~~~~~~~~~~*/
